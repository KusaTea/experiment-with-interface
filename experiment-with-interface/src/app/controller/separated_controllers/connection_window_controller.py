...
from data.repositories import SettingsData, SensogloveModule, SensogloveDataHandler, QuattrocentoModule, QuattrocentoDataHandler, QuattrocentoSettings


class ConnectionWindowController(QObject):

    finished = Signal()
    back_button_pushed = Signal()
    start_record_button_pushed = Signal()
    ...
    def __init__(
        ...
        settings_updated.connect(self.__update_settings)
        ...
        self.__quattrocento_settings = QuattrocentoSettings(self.__settings['myograph_settings']['options'])
        self.__quattrocento_module = QuattrocentoModule(
            ip_address=self.__settings['myograph_settings']['ip'],
        ...
        self.__connection_window = window
        ...
        stop_signal.connect(self.__stop_record)
    ...
    def __connect_callback(self):
        try:
            if not self.__is_sensoglove_connected:
                self.__sensoglove_data_handler = SensogloveDataHandler(
                    ...
                    save_folder_dir=self.__dirs_store.raw_data_dir,
            ...
            del self.__sensoglove_data_handler
            self.__is_sensoglove_connected = False
        ...
        try:
            ...
            del self.__quattrocento_data_handler
            self.__is_quattrocento_connected = False
    ...
    def __start_record_callback(self):
        if self.__sensoglove_data_handler.is_ready and self.__quattrocento_data_handler.is_ready:
            ...
            self.start_record_button_pushed.emit()
